...
Текущий набор собран скриптом `scripts/build-gifs.sh`: он берёт из открытой базы [free-exercise-db](https://github.com/yuhonas/free-exercise-db) пары кадров «начальная позиция / конечная» и склеивает их `ffmpeg` в двухкадровые циклы. Анимированных GIF со внятной лицензией в открытом доступе нет, а пара кадров движение передаёт.

Пять гифок приблизительные — движение то же, но снаряд или вариант другой: `bulgarian-split-squat`, `bosu-balance`, `band-rotation`, `medball-slam`, `kettlebell-swing`. Координационной лестницы в базе нет вовсе. Эти шесть стоит заменить своей съёмкой, когда дойдут руки.
